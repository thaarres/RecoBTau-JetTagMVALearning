...
parser.add_option("-f", "--valInputFile", dest="valPath",
                  help="Read input file for sample to validated", metavar="VALFILE")
parser.add_option("-F", "--refInputFile", dest="refPath",
...
print options.valPath, options.refPath, options.ValRel, options.RefRel, options.ValSample, options.RefSample, options.batch, options.drawLegend, options.printBanner, options.Banner, options.doRatio
...
for b in EtaPtBin :
    ...
    for h in listHistos :
        ...
        for tag in h.listTagger :
            ...
            if h.doPerformance :
                keyHisto = tag+"_performance"
            #loop over the flavours
            h_Val = {}
            h_Ref = {}
            for f in listFlavors :
                path = pathInFile+tag+"_"+b+"/"+h.name+"_"+tag+"_"+b+f
                #for performance plots this doesn't do anything
                if "_B_" in path : 
            ...
            if options.doRatio :
                if h.doPerformance:
                    ratiosList = createRatioFromGraph(valHistos[keyHisto],refHistos[keyHisto]) #None
                else :
            ...
            if h.doPerformance : saveName+="_vs"+h.tagFlavor 
            #save canvas
