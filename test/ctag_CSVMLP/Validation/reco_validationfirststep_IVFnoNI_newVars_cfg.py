...
process.BTauMVAJetTagComputerRecord = cms.ESSource("PoolDBESSource",
	...
	connect = cms.string("sqlite_file:MVAJetTags_IVFnoNI_finalcuts_100DUSG_0B_ver5_3_14.db"),
...
process.combinedSecondaryVertexCT=process.combinedSecondaryVertexV2.clone(
...
process.ak5JetCleanedTracksAssociatorAtVertex.jets = cms.InputTag("ak5PFJets")


# cut on decay length
process.inclusiveVertexFinder.vertexMinDLen2DSig = cms.double(0.) #2.5 sigma
process.inclusiveVertexFinder.vertexMinDLenSig = cms.double(0.) #0.5 sigma
process.inclusiveVertexFinder.seedMin3DIPSignificance = cms.double(1.0) # default 1.2
process.inclusiveVertexFinder.seedMin3DIPValue = cms.double(0.005) # default 0.005
process.inclusiveVertexFinder.clusterScale = cms.double(0.5) # default 1.

process.inclusiveVertexFinder2 = process.inclusiveVertexFinder.clone(tracks = cms.InputTag("trackCollectionCleaner"))
# cut on decay length after the clean NI
process.inclusiveVertexFinder2.vertexMinDLen2DSig = cms.double(0.) #2.5 sigma
process.inclusiveVertexFinder2.vertexMinDLenSig = cms.double(0.) #0.5 sigma
process.inclusiveVertexFinder2.seedMin3DIPSignificance = cms.double(1.0) # default 1.2
process.inclusiveVertexFinder2.seedMin3DIPValue = cms.double(0.005) # default 0.005
process.inclusiveVertexFinder2.clusterScale = cms.double(0.5) # default 1.
...
process.goodOfflinePrimaryVertices2 = cms.EDFilter(
       "PrimaryVertexObjectFilter",
          filterParams = pvSelector.clone( minNdof = cms.double(4.0), maxZ = cms.double(24.0) ),
...
process.inclusiveVertexFinder2.primaryVertices = cms.InputTag("goodOfflinePrimaryVertices2")
process.trackVertexArbitrator2.primaryVertices = cms.InputTag("goodOfflinePrimaryVertices2")
...
process.cleanedImpactParameterTagInfos.primaryVertex = cms.InputTag("goodOfflinePrimaryVertices2")
...
process.inclusiveSecondaryVertexFinderTagInfos.vertexCuts.distSig2dMin = 0. # default value 3.0 to release cuts on flight dist
process.cleanedInclusiveSecondaryVertexFinderTagInfos.vertexCuts.distSig2dMin = 0. # same

#process.inclusiveSecondaryVertexFinderTagInfos.vertexCuts.distSig2dMin = 0.002 # default value 0.01 to release cuts on flight dist
#process.cleanedInclusiveSecondaryVertexFinderTagInfos.vertexCuts.distVal2dMin = 0.002 # same



# this has to have the same tag as the config above ! 
process.myIVFCombinedSecondaryVertexCJetTags = process.combinedInclusiveSecondaryVertexBJetTags.clone(
        jetTagComputer = cms.string("combinedSecondaryVertexCT"),
...
# BEFORE adding the nucl. int. cleaner
#process.myIVFCombinedSecondaryVertexCJetTags = process.combinedSecondaryVertexV2BJetTags.clone(
#  jetTagComputer = cms.string('combinedSecondaryVertexCT'),
...
process.nuclearInteractionIdentifier *
process.cleanedInclusiveMergedVertices *
process.trackCollectionCleaner *
process.offlinePrimaryVertices2*
process.goodOfflinePrimaryVertices2 *
process.inclusiveVertexing2 *
process.ak5JetCleanedTracksAssociatorAtVertex *
process.cleanedImpactParameterTagInfos *
process.cleanedInclusiveSecondaryVertexFinderTagInfos *
#process.flavourSeq *
process.myIVFCombinedSecondaryVertexCJetTags)
...
process.CustombTagValidation = process.bTagValidation.clone(
    tagConfig = cms.VPSet(
        cms.PSet(
            ...
            label = cms.InputTag("myIVFCombinedSecondaryVertexCJetTags"),
            folder = cms.string("CSVIVFV2ctag")
        ) 
			),
      finalizePlots = False,
      finalizeOnly = False
