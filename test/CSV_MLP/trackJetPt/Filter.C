...
// Example of Root macro based on $ROOTSYS/tutorials/tree/copytree3.C
  ...
  gSystem->Load("$ROOTSYS/test/libEvent");
	...
	Float_t ptbins[] = {0,40,60,90,150,400,600,10000};
	...
	for(int k = 2; k<3; k++){
  	...
  	oldtree->SetBranchAddress("trackJetPt",&jetpt);
  	oldtree->SetBranchAddress("jetEta",&jeteta);

  	//Create a new file + a clone of old tree in new file
  	TFile *newfile = new TFile("skimmed_20k_eachtrackjetptetabin_"+name,"recreate");
  	TTree *newtree = oldtree->CloneTree(0);
		...
		if(filename[k].find("DUSG") != std::string::npos )// DUSG file found!!!
		{
			...
			for(int j = 0 ; j<19 ; j++)
			{
				cout << "Tree " << filename[k] << " has " << nbOfjets_DUS[j] << " in trackjetpt/eta-bin " << j << " and we keep " << nbOfjetsKept_DUS[j] << endl;
				cout << "Tree " << filename[k] << " has " << nbOfjets_G[j] << " in trackjetpt/eta-bin " << j << " and we keep " << nbOfjetsKept_G[j] << endl;
			}
			newtree->Print();
	  	newtree->AutoSave();
		}
...
//					cout << "entry " << i << " in 19th pt/eta bin"  << endl;
			...
			for(int j = 0 ; j<19 ; j++)
			{
				cout << "Tree " << filename[k] << " has " << nbOfjets[j] << " in trackjetpt/eta-bin " << j << " and we keep " << nbOfjetsKept[j] << endl;
			}
			newtree->Print();
	  	newtree->AutoSave();
